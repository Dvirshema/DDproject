 ...
 function RanderEmployees(EmpArr) {
     ...
     EmpArr.forEach(e => {
       ...
       let statusText = e.empStatus ? 'פעיל' : 'לא פעיל'; // set text based on empStatus
         ...
         str += `<button onclick="UpdateEmployee(${e.empNum})" id="update${e.empNum}" style="background-color: #412de4; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer;">Update</button>`;
  ...
  function GetByStatus(EmpArr){
    ...
    if (employee.empStatus==booleanValue) {     
      ...
      str += `<button onclick="UpdateEmployee(${employee.empNum})" id="update${employee.empNum}" style="background-color: #412de4; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer;">Update</button>`;
      str += `<h4 class="status-style" style="color: ${statusColor}"><strong>${statusText}</strong></h4>`;
      str += `</div></div>`; 
    }
    });   
  ...
  function UpdateEmployee(empNum){
    ...
    location.assign('../HTML/EditDetailsPageAdmin.html'); 
  }
